%OPeNDAP_MATLAB_AdvancedExample.m
...
%=============================================
	pathDir='Inside-dev1.nkn.uidaho.edu:8080/thredds/dodsC/'; %(this is for MACAv1-METDATA only)
%=============================================
%      GET LAT/LON INDICES FOR GEOGRAPHICAL SUBSET
%=============================================
	%look at sample file
	pathname=[pathDir,'agg_maca_huss_BNU-ESM_r1i1p1_historical_1950_2005_WUSA.nc'];
...
%=============================================
	...
	m.data = NaN(length(lat),length(lon),365,length(years_hist)+length(years_fut),length(exp_target),length(model_target),length(var_target));
	...
	for var=1:length(var_target);
		for model = 1:length(model_target);
			...
			myURL=[pathDir,'agg_maca_',char(VAR_NAME(var_target(var))),'_',char(MODEL_NAME(model_target(model))),'_',...
			...
			%need to access just a small time slice first.. before getting the data (solution credit:Ian Pfingsten)
